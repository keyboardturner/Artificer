...
local actionBarMappings = { -- required for the specific action info IDs
	...
	for _, barInfo in ipairs(actionBarMappings) do
		for i = barInfo.start, barInfo.stop do
			...
			if button then
				...
				if actionType == "outfit" and Artificer_DB and Artificer_DB.Widgets.OutfitIcon then
...
Artificer.UpdateOutfitHighlighter = UpdateOutfitHighlighter;
